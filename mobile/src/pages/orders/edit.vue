      ...
      <view class="form-section card">
        ...
        <view v-if="selectedCustomer" class="customer-detail">
          ...
          <view class="customer-salesman-row">
            ...
            <text class="customer-salesman-value">{{ selectedCustomer.salesman_name || '无' }}</text>
          </view>
        </view>
          ...
          <view v-for="(product, pIndex) in form.products" :key="pIndex" class="product-card" @tap="openProductSelect(pIndex)">
            ...
            <view class="pc-row pc-row--price" @tap.stop>
              <view class="pc-qty">
                ...
                <text v-if="product.unit" class="pc-unit">{{ product.unit }}</text>
              </view>
  ...
  setup() {
    ...
    const loadOrderData = async (id) => {
      try {
        ...
        // 回填基本信息
        form.value.customerId = orderData.customer_id || null
        form.value.customerName = orderData.customer_name || ''
        form.value.customerPhone = orderData.customer_phone || ''
        form.value.customerAddress = orderData.customer_address || ''
        form.value.specialApproval = orderData.is_special_approved === 1
        ...
        // 回填同行信息
        if (orderData.peer_id) {
          form.value.peerId = orderData.peer_id
          if (orderData.peer) {
            form.value.companionName = orderData.peer.peer_name || ''
            form.value.companionPhone = orderData.peer.phone || ''
          }
          showCompanion.value = true
        }

        // 回填订单备注
        form.value.remark = orderData.remark || ''

        // 回填商品列表
        form.value.products = items.map(item => ({
          itemId: item.id || 0,  // 保留 order_item 的 ID
          skuId: item.sku_id || null,
          name: item.product_name || '',
          sku: item.sku_name || '',
          quantity: item.quantity || 1,
          listPrice: Number(item.list_price) || 0,
          minPrice: 0, // 编辑时无法获取最低价，设为0不限制
        ...
        if (gifts.length > 0) {
          showGift.value = true
          form.value.gifts = gifts.map(gift => ({
            giftId: gift.gift_id || null,
            name: gift.gift_name || gift.name || '',
            costPrice: Number(gift.cost_price) || 0,
            stockQuantity: 0,
    ...
    const onProductSelect = (product) => {
      ...
      if (index >= 0 && index < form.value.products.length) {
        ...
        form.value.products[index].unit = product.product?.unit || '件'
      }
    }
      ...
      try {
        const orderData = {
          ...
          items: form.value.products.map(p => ({
            item_id: p.itemId || 0,  // 已有商品传ID，新增商品传0
            sku_id: p.skuId || 0,
            product_name: p.name,
